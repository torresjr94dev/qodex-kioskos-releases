...
#   QODEX_CALL_MAX_S=300           failsafe FINAL: tope de video con relé activo
#                                  (0 = sin tope). El cierre normal al colgar lo da
#                                  el listener AMI propio (Hangup), no este tope.
...
Flujo de la llamada de emergencia:
- El rele (QODEX_GPIO_PIN, BCM, default 22; pull-up) se activa con la
  llamada; mientras este activo y haya una URL rtsp:// valida en
  rtsp_url.txt (la escribe config_editor al detectar BridgeEnter via AMI),
  se muestra la camara fullscreen ENCIMA del kiosko.
- FIN de llamada — el video se cierra por CUALQUIERA de estas senales:
    1. Hangup en AMI de una llamada de este totem (listener propio, en un
       hilo: el conmutador NO desactiva el rele al colgar, asi que esta es
       la senal principal). Ademas vacia rtsp_url.txt.
    2. El rele se suelta (comportamiento original).
    3. rtsp_url.txt se limpia externamente con el rele activo.
    4. Failsafe QODEX_CALL_MAX_S (default 300 s, 0 = sin tope) por si todas
       las senales anteriores fallan.
  Tras un cierre con rele activo, la misma URL no se relanza; una llamada
  NUEVA (URL re-escrita) si se muestra aunque el rele siga trabado.

Reproductor: ffplay con RENDERIZADO POR SOFTWARE (la salida por hardware
pinta negro con el kiosko Electron activo). Fallback: VLC x11 software.
QODEX_CAM_ROTATE (0|90|180|270, default 90) rota el video al montaje.

Todo queda en journal (journalctl -u qodex-gpio): boton, AMI, rtsp_url.txt
y arranque/muerte del reproductor.
"""
...
if CAM_ROTATE not in ("0", "90", "180", "270"):
    CAM_ROTATE = "90"
CALL_MAX_S = int(os.environ.get("QODEX_CALL_MAX_S", "300"))
# Fuente de configuracion (credenciales AMI + mapeo boton->IP). SIN secretos
# hardcodeados: se toma de la env QODEX_SIP_CONFIG_URL o, en su defecto, se
# lee del config_editor.py de TI presente en el dispositivo (solo lectura) —
# asi el token nunca viaja en nuestro codigo publicado.
CONFIG_EDITOR_PY = os.path.expanduser("~/Desktop/config_editor.py")
AMI_CONFIG_REFRESH_S = 300


def resolve_sip_config_url():
    url = os.environ.get("QODEX_SIP_CONFIG_URL", "").strip()
    if url:
        return url
    try:
        import re
        with open(CONFIG_EDITOR_PY, encoding="utf-8", errors="ignore") as f:
            m = re.search(r'CAMERA_CONFIG_URL\s*=\s*["\']([^"\']+)["\']', f.read())
        if m:
            return m.group(1)
    except OSError:
        pass
...
SIP_CONFIG_URL = resolve_sip_config_url()
...
stale_url = None
# Señal puesta por el hilo AMI cuando cuelga una llamada de ESTE totem.
...
def clear_rtsp(reason):
    try:
        with open(RTSP_FILE, "w") as f:
            f.write("")
        log(f"rtsp_url.txt vaciado ({reason})")
    except OSError as e:
...
def back_to_kiosk(reason):
    """Cerrar camara, limpiar RTSP y refrescar el listener AMI de TI."""
    kill_player()
    clear_rtsp(reason)
...
# ==========================================================================
# Listener AMI propio — detecta el Hangup de las llamadas de ESTE totem.
# El conmutador no desactiva el rele al colgar, asi que esta es la senal
# principal de fin de llamada. Solo LECTURA de eventos; nunca modifica nada
# del lado de TI.
# ==========================================================================
...
def local_ipv4s():
    ...
    try:
        ...
        log(f"[ami] AVISO: no se pudieron leer IPs locales: {e}")
    return ips


def fetch_sip_config():
    """Config de TI (solo lectura): credenciales AMI + botones_fisicos."""
    url = SIP_CONFIG_URL or resolve_sip_config_url()
    if not url:
        raise RuntimeError(
            "sin URL de sip-config (define QODEX_SIP_CONFIG_URL o instala config_editor.py)"
        )
    with urllib.request.urlopen(url, timeout=8) as resp:
        return json.loads(resp.read().decode())


def our_button_extensions(config, ips):
    exts = set()
    for ext, info in (config.get("botones_fisicos") or {}).items():
        if isinstance(info, dict) and info.get("iprpi") in ips:
            exts.add(str(ext))
    return exts


def ami_hangup_listener():
    while True:
        try:
            config = fetch_sip_config()
            host = config.get("servidorIP")
            port = int(config.get("ami_port") or 0)
            user = config.get("ami_user")
            secret = config.get("ami_pass")
            if not (host and port and user and secret):
                log("[ami] configuracion AMI incompleta — reintento en 15s")
                time.sleep(15)
                continue
            ips = local_ipv4s()
            exts = our_button_extensions(config, ips)
            if not exts:
                log(f"[ami] AVISO: ningun boton mapeado a estas IPs {sorted(ips)} — reintento en 60s")
                time.sleep(60)
                continue
            ...
            s.settimeout(10)
            s.connect((host, port))
            saludo = s.recv(1024).decode(errors="ignore")
            if "Asterisk" not in saludo:
                raise ConnectionError("sin saludo de Asterisk")
            s.send(f"Action: Login\nUsername: {user}\nSecret: {secret}\nEvents: on\n\n".encode())
            login = s.recv(2048).decode(errors="ignore")
            if "Success" not in login:
                raise ConnectionError("login AMI rechazado")
            log(f"[ami] conectado a {host}:{port}; botones de este totem: {sorted(exts)}")

            buffer = ""
            last_event = time.time()
            last_refresh = time.time()
            tracked = {}  # linkedid -> ts de llamadas de este totem

            while True:
                try:
                    data = s.recv(8192).decode(errors="ignore")
                except socket.timeout:
                    if time.time() - last_event > 90:
                        raise ConnectionError("AMI inactivo >90s")
                    continue
                if not data:
                    raise ConnectionError("conexion AMI cerrada")
                buffer += data
                ...
                while "\r\n\r\n" in buffer:
                    ...
                    involved = {
                        campos.get("CallerIDNum"),
                        campos.get("ConnectedLineNum"),
                        campos.get("Exten"),
                    } & exts
                    linked = campos.get("Linkedid") or campos.get("Uniqueid")

                    if involved and linked:
                        if campos.get("Event") in ("BridgeEnter", "Newchannel", "DialBegin"):
                            if linked not in tracked:
                                log(f"[ami] llamada de este totem detectada (ext {sorted(involved)}, id {linked})")
                            tracked[linked] = time.time()

                    if campos.get("Event") == "Hangup":
                        es_nuestra = bool(involved) or (linked in tracked)
                        if es_nuestra:
                            log(f"[ami] HANGUP de llamada de este totem (id {linked}, causa {campos.get('Cause-txt') or campos.get('Cause')})")
                            tracked.pop(linked, None)
                            clear_rtsp("hangup AMI")
                            hangup_event.set()

                    # poda de llamadas viejas
                    if len(tracked) > 32:
                        cutoff = time.time() - 3600
                        for k in [k for k, ts in tracked.items() if ts < cutoff]:
                            tracked.pop(k, None)

                if time.time() - last_refresh > AMI_CONFIG_REFRESH_S:
                    new_exts = our_button_extensions(fetch_sip_config(), local_ipv4s())
                    if new_exts:
                        if new_exts != exts:
                            log(f"[ami] botones actualizados: {sorted(new_exts)}")
                        exts = new_exts
                    last_refresh = time.time()

        except Exception as e:
            log(f"[ami] error: {e} — reconexion en 5s")
            try:
                s.close()
            except Exception:
                pass
...
threading.Thread(target=ami_hangup_listener, daemon=True).start()

log(
    f"supervisor iniciado (GPIO {GPIO_PIN}, pull-up, rotacion camara {CAM_ROTATE}°, "
    f"hangup por AMI activo, failsafe {CALL_MAX_S}s)"
)
...
try:
    while True:
        ...
        if pressed:
            ...
            # Fin de llamada, senal principal: Hangup detectado en AMI.
            if hangup_event.is_set():
                hangup_event.clear()
                if player_running:
                    log("cerrando video por HANGUP (rele aun activo)")
                    kill_player()
            ...
            # Fin de llamada: alguien limpio la URL con el rele activo.
            if player_running and not valid:
                log("RTSP limpiado con el rele aun activo — cerrando video")
                kill_player()
                player_running = False
                current_url = None
                stale_url = None

            # Failsafe final: tope de duracion.
            elif player_running and CALL_MAX_S > 0 and (now - player_started_at) > CALL_MAX_S:
                log(f"tope de llamada alcanzado ({CALL_MAX_S}s) con el rele aun activo — cerrando video (failsafe)")
                kill_player()
                player_running = False
                stale_url = current_url
                current_url = None

            if stale_url is not None and url != stale_url:
                stale_url = None

            launch_blocked = stale_url is not None and url == stale_url
            if valid and not player_running and not launch_blocked and (now - player_died_at) >= PLAYER_RESPAWN_DELAY_S:
                kill_player()
                ...
                current_url = url
                player_started_at = now
            elif not valid and not player_running and (now - last_wait_log) >= WAIT_LOG_EVERY_S:
                log(f"esperando RTSP valido... (contenido actual: {url!r})")
                last_wait_log = now

        elif was_pressed:
            log("boton SOLTADO — volviendo al kiosko")
            back_to_kiosk("boton soltado")
            current_url = None
            stale_url = None
            hangup_event.clear()
...
Environment=QODEX_CALL_MAX_S=${QODEX_CALL_MAX_S:-300}
...
# El listener AMI de TI (config_editor) escribe la URL de la camara cuando
# entra la llamada de emergencia; sin el, el boton nunca muestra video. Si
# existe pero esta deshabilitado (imagen recien aprovisionada), se habilita.
# Solo se activa el servicio — sus archivos no se tocan.
if systemctl list-unit-files config_editor.service >/dev/null 2>&1; then
  if ! systemctl is-enabled config_editor.service >/dev/null 2>&1; then
    systemctl enable --now config_editor.service 2>/dev/null       && echo "   config_editor habilitado (listener AMI de la llamada)"       || echo "   AVISO: no se pudo habilitar config_editor"
  fi
fi
